...
import { StockData } from './actions/alert.actions';

export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
}

type AlertType = "price" | "percent";
...
export const checkAlertCondition =(
  ...
  try {
    const target =
      type === "price" ? stock.price : stock.percent;

    if (condition === "gt") return target > value;
    if (condition === "lt") return target < value;

    return false;
  } catch (error) {
    console.error("checkAlertCondition error:", error);
    return false;
  }
};
