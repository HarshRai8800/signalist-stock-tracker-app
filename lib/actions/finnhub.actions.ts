...
export async function getCompanyNews(
  ...
  try {
    const range = getDateRange(2); // shorter range for alerts
    const token =
    ...
    // 🔹 SAME validation + formatting (no change)
    const filtered = articles
      .filter(validateArticle)
      .slice(0, 5) // small batch for alerts
      .map((article, idx) =>
        formatArticle(article, true, cleanSymbol, idx)
      );

    // 🔹 sort latest first (same as your logic)
